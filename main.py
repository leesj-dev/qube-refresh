            ...
            if len(questions) > solved:
                ...
                # Case 2-2-2. 다른 마스터가 답변 중이지 않을 때
                    ...
                    # Case 2-2-2-2. 내가 답변하지 않은 문제라면
                        ...
                        # Step 2. 문제 본문 & 이미지 보내기
                        images = driver.find_elements(By.ID, "net.megastudy.qube:id/iv_chat_image")
                        for i in range(0, len(images)):
                            img_num = str(i + 1)
                            file_name = os.path.join("images", f"{current_time}-{img_num}.png")
                            with open(file_name, "wb") as screenshot:
                                screenshot.write(images[i].screenshot_as_png)
                        ...
                        while proceed.content in range(1, 4):  # 제대로 된 input값이 들어올 때까지 반복
                            proceed = int(await client.wait_for("message", timeout=120.0))
                            print(proceed.content)

                            if proceed.content == 1:
                                await channel.send("보내실 답을 입력해주세요.")
                                answer = await client.wait_for("message", timeout=120.0)
                                driver.find_element(By.ID, "net.megastudy.qube:id/et_input_text").send_keys(answer)
